...
def setup_trackbars():
    ...
    cv2.createTrackbar('Dilate', 'Trackbars', 0, 1, callback)
    cv2.createTrackbar('Dilate K', 'Trackbars', 0, 9, callback)
...
def get_trackbar_values():
    ...
    values.append(cv2.getTrackbarPos("Dilate", "Trackbars"))
    values.append(cv2.getTrackbarPos("Dilate K", "Trackbars"))
    return values
...
def filter_binarize_waste_removal(raw_image):
    gauss_bil, kernel, adapt_otsu, waste, dilate, dilate_k = get_trackbar_values()
    if kernel % 2 == 0:
        kernel += 1
    try:
        if gauss_bil == 0:
